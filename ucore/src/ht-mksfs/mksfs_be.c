...
static void inode_big_endian(struct inode *inode)
{
	...
	case SFS_TYPE_FILE:
	case SFS_TYPE_LINK:
		htobe(be_inode, inode, fileinfo.size, 32);
		break;
	case SFS_TYPE_DIR:
		htobe(be_inode, inode, dirinfo.slots, 32);
		htobe(be_inode, inode, dirinfo.parent, 32);
		break;
	}
...
static void super_big_endian(struct super_block *super)
{
...
static void entry_big_endian(struct sfs_entry *entry)
{
...
static void cache_block_big_endian(void *cache)
{
	uint32_t *be_block_entry = (uint32_t *) endian_buffer;
	uint32_t *original_entry = (uint32_t *) cache;

	int i;
	for (i = 0; i < SFS_BLKSIZE / sizeof(uint32_t);
	     i++, be_block_entry++, original_entry++)
...
void close_sfs(struct sfs_fs *sfs)
{
	...
	for (i = 0; i < ninos; ino++, i += SFS_BLKBITS) {
		...
		//write_block(sfs, buffer, sizeof(buffer), ino);
		{
			...
			write_block(sfs, endian_buffer, sizeof(endian_buffer),
				    ino);
		}
	}
	//write_block(sfs, &(sfs->super), sizeof(sfs->super), SFS_BLKN_SUPER);
	{
		...
		write_block(sfs, endian_buffer, sizeof(sfs->super),
			    SFS_BLKN_SUPER);
	}
